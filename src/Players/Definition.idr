...
public export
Player : (profiles : Type) -> (actions : Type) -> (utility : actions -> Type) -> Type
Player profiles actions utility = DLens (MkCo profiles (\p => profiles -> Bool)) (MkCo actions (\a => (a1 : actions) -> utility a1))

public export
nash : Player p1 a1 u1 -> Player p2 a2 u2
     -> Player (p1, p2) (a1, a2) (\aa => (u1 (fst aa), u2 (snd aa)))
